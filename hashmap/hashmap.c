...
{
	// YOUR CODE GOES HERE
	struct binEntry *remove_kv;
	remove_kv = lookup(key);

	if (remove_kv != NULL){
		if (remove_kv -> next != NULL && remove_kv -> prev != NULL){
			remove_kv -> prev -> next = remove_kv -> next;
			remove_kv -> next -> prev = remove_kv -> prev;
		}
		
		else if (remove_kv -> next != NULL){
			remove_kv -> next -> prev = NULL;
			hashtable[hash(key)] = remove_kv -> next;
		}

		else if (remove_kv -> prev != NULL){
			remove_kv -> prev -> next = NULL;
		}
